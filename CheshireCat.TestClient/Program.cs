...
using CheshireCat.Library;

namespace CheshireCat.TestClient
{
    ...
    class MainClass
    {
        ...
        public const int FRAME_SIZE = 200;
        ...
        public static void LEDTestSequence(UdpClient client)
        {
            ...
            Byte[] clearFrame = sendSingleColorFrame(FRAME_SIZE, RGBColors.Black);
            client.SendAsync(clearFrame, clearFrame.Length, UDP_HOST, UDP_PORT).GetAwaiter();
            WaitResponse(1000, false);

            // Purple
            Console.WriteLine("Purple...");
            Byte[] purples = sendSingleColorFrame(FRAME_SIZE, RGBColors.Purple);
            client.SendAsync(purples, purples.Length, UDP_HOST, UDP_PORT).GetAwaiter();
            WaitResponse(1000, false);

            //Moving pixel
            Console.WriteLine("Moving pixel...");
            client.SendAsync(clearFrame, clearFrame.Length, UDP_HOST, UDP_PORT).GetAwaiter(); // Clear frame first
            Byte[] moveSinglePixelFrame;
            for (int i = 1; i <= FRAME_SIZE; i++)
            {
                moveSinglePixelFrame = sendSinglePixelFrame(FRAME_SIZE, i, RGBColors.Red);
                client.SendAsync(moveSinglePixelFrame, moveSinglePixelFrame.Length, UDP_HOST, UDP_PORT);
                Thread.Sleep(20);
            }
            WaitResponse(1000, false);
            ...
            for (int i = 1; i <= (iterations * frameSequenceLength); i++)
            {
                ...
                if (i % 3 == 0)
                    shiftSinglePixelFrame = sendSinglePixelFrame(FRAME_SIZE, i % (FRAME_SIZE), RGBColors.Purple);
                else if (i % 3 == 1)
                    shiftSinglePixelFrame = sendSinglePixelFrame(FRAME_SIZE, i % (FRAME_SIZE), RGBColors.Orange);
                else
                    shiftSinglePixelFrame = sendSinglePixelFrame(FRAME_SIZE, i % (FRAME_SIZE), RGBColors.Aqua);

                // TODO: UDP frame size may be different from LED frame
                // LED frames might be large enought to require multiple UDP frames
                client.SendAsync(shiftSinglePixelFrame, shiftSinglePixelFrame.Length, UDP_HOST, UDP_PORT);
        ...
        public static void LEDTestTwinkle(UdpClient client)
        {
            Console.WriteLine($"UDP Host: {UDP_HOST}");
            Console.WriteLine($"UDP Port: {UDP_PORT}");
            Console.WriteLine($"Frame Size: {FRAME_SIZE}");
       ...
       //     WaitResponse(2000, true);
            ...
            // Purple
            Console.WriteLine("Purple...");
            Byte[] purples = sendSingleColorFrame(FRAME_SIZE, RGBColors.Red);
            client.SendAsync(purples, purples.Length, UDP_HOST, UDP_PORT).GetAwaiter();
            WaitResponse(2000, true);

            var effects = new LEDEffects();
            
            for (int i = 1; i <= (iterations); i++)
            {
                ledFrame = effects.Twinkle(FRAME_SIZE, RGBPalettes.OCEAN);
                client.SendAsync(ledFrame, ledFrame.Length, UDP_HOST, UDP_PORT).GetAwaiter();
                Thread.Sleep(pixelSleep);
            }
        ...
        {
            var endpoint = new IPEndPoint(IPAddress.Parse(UDP_HOST), UDP_PORT);
            ...
            try
            {
                ...
                Console.ReadKey();
                Console.WriteLine("Starting LED test effect (twinkle)...");
                LEDTestTwinkle(udpClient);
                Console.ReadKey();
            }
            catch (Exception e)
            {
        ...
        /// 
        /// Original contributor: Phillipa Gill 
        /// https://github.com/phillipa/LED-cube
        /// </summary>
